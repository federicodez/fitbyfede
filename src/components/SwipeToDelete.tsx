...
const SwipeToDelete = ({
  ...
  useEffect(() => {
    ...
    root?.style.setProperty("30px", height + "px");
    root?.style.setProperty("250ms", transitionDuration + "ms");
    root?.style.setProperty("0", rtl ? "1" : "-1");
    root?.style.setProperty("rgba(252, 58, 48, 1)", deleteColor);
    root?.style.setProperty("75px", deleteWidth + "px");
  }, [deleteColor, deleteWidth, height, rtl, transitionDuration]);

  useEffect(() => {
    const root = container.current;
    root?.style.setProperty("0px", translate * (rtl ? -1 : 1) + "px");
